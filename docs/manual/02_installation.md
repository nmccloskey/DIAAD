...
1. Visit [**https://diaad.streamlit.app/**](https://diaad.streamlit.app/)  
...
### 1.2  Installing DIAAD Locally (Command Line Interface)
...
(If you prefer `venv`: `python -m venv diaad && source diaad/bin/activate`)

#### Step 2 – Install DIAAD
...
your_project/
  ...
  diaad_data/
    input/            # Place CHAT (.cha) or Excel input files here
                          # DIAAD automatically creates 'output/' on run
```
...
```bash
pip uninstall diaad
conda remove --name diaad --all   # optional, removes environment
```
