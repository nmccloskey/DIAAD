...
TEMPLATE_OUTPUT_DIRS = {
    ...
    "combine": "templates_combine",
}
TEMPLATE_SUBSET_INPUT_DIRS = {
    "subset": "sample_subset",
    "resubset": "sample_resubset",
}
TEMPLATE_COMBINATION_INPUT_DIR = "template_combination"
CU_OUTPUT_DIRS = {
    "files": "cus_files",
...
EXPECTED_OUTPUT_ROOTS_BY_COMMAND_ID: dict[str, str] = {
    ...
    "templates.combine": expected_output_root(
        TEMPLATES_MODULE_DIR,
        TEMPLATE_OUTPUT_DIRS["combine"],
    ),
...
EXPECTED_OUTPUT_RUNTIME_ROOTS: dict[str, tuple[str, ...]] = {
    ...
    EXPECTED_OUTPUT_ROOTS_BY_COMMAND_ID["templates.combine"]: (
        "coding_templates",
    ),
...
def _normalize_example_commands(commands: Iterable[str] | str) -> tuple[str, ...]:
    ...
    individual_commands = {
        "templates subset": (
            "the command expects exactly one Excel workbook under example_input"
        ),
        "templates combine": (
            "the command recursively combines every Excel workbook under "
            "example_input"
        ),
    }
    for command, reason in individual_commands.items():
        if command in normalized and len(normalized) > 1:
            raise ValueError(
                f"The '{command}' command example must be generated by itself "
                f"because {reason}."
            )
...
def _materialize_inputs(project_dir: Path, specs: dict[str, dict[str, Any]], *, force: bool) -> None:
    ...
    _write_template_combination_inputs(project_dir, specs, force=force)
...
def _template_combination_workbooks(
    specs: dict[str, dict[str, Any]],
) -> dict[str, dict[str, pd.DataFrame]]:
    sample_id_field = specs["advanced_config"].get("sample_id_column", "sample_id")
    return {
        "site_a/template_batch_a.xlsx": {
            "ratings": pd.DataFrame(
                [
                    {
                        sample_id_field: "P1_picnic_pre",
                        "coder": "coder_a",
                        "engagement_code": 2,
                    },
                    {
                        sample_id_field: "P2_picnic_pre",
                        "coder": "coder_a",
                        "engagement_code": 1,
                    },
                ]
        ...
        "site_b/template_batch_b.xlsx": {
            "ratings": pd.DataFrame(
                [
                    {
                        sample_id_field: "P1_picnic_post",
                        "coder": "coder_b",
                        "engagement_code": 3,
                    }
                ]
...
def _write_template_combination_workbooks(
    input_dir: Path,
    specs: dict[str, dict[str, Any]],
    *,
    force: bool,
) -> None:
    for relative_path, sheets in _template_combination_workbooks(specs).items():
...
def _materialize_template_combination_capability(ctx: ExampleBuildContext) -> None:
    _write_template_combination_workbooks(
...
def _template_combination_input_dir(
    project_dir: Path,
    specs: dict[str, dict[str, Any]],
) -> Path:
    return (
...
def _write_template_combination_inputs(
    project_dir: Path,
    specs: dict[str, dict[str, Any]],
    *,
    force: bool,
    ...
    _write_template_combination_workbooks(
        _template_combination_input_dir(project_dir, specs),
        specs,
        force=force,
    )
...
def _write_expected_template_combination(
    project_dir: Path,
    specs: dict[str, dict[str, Any]],
    *,
    force: bool,
    ...
    with scratch_dir(project_dir) as tmpdir:
        make_combined_template_file(
            input_dir=_template_combination_input_dir(project_dir, specs),
            output_dir=tmpdir,
        )
        replace_tree(tmpdir / "coding_templates", expected_dir, force=force)
...
def _build_templates_combine_example_output(ctx: ExampleBuildContext) -> None:
    make_combined_template_file(
        input_dir=ctx.example_input_dir,
        output_dir=ctx.example_output_dir,
    )
...
EXAMPLE_CAPABILITIES: dict[str, ExampleCapability] = {
    ...
    "template_combination_workbooks": ExampleCapability(
        name="template_combination_workbooks",
        materialize=_materialize_template_combination_capability,
    ),
...
EXAMPLE_COMMAND_PLANS: dict[str, ExampleCommandPlan] = {
    ...
    "templates combine": ExampleCommandPlan(
        command="templates combine",
        required_capabilities=("template_combination_workbooks",),
        build_output=_build_templates_combine_example_output,
    ),
...
def _generate_full_example_files(destination: str | Path, *, force: bool) -> Path:
    ...
    _write_expected_template_combination(project_dir, specs, force=force)
